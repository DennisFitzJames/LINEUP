...
latestFileName = "LX02_Latest.txt"
latestFilePath = exportPath & "\" & latestFileName
timestampText = TimestampForFile()
timestampFileName = "LX02_" & timestampText & ".txt"
...
session.findById("wnd[0]/usr/ctxtS1_LGTYP-LOW").Text = "924"
session.findById("wnd[0]/tbar[1]/btn[8]").press
WaitForSAPReady session, 60
...
If Not WaitForExport(latestFilePath, 60) Then
    Fail "LX02 text export did not become ready within 60 seconds: " & latestFilePath
End If

ArchiveFile latestFilePath, archivePath & "\" & timestampFileName

WScript.Echo "LX02 text export complete:"
...
Sub CheckError(stepName)
    If Err.Number <> 0 Then
        ...
        Err.Clear
        Fail "LX02 failed while " & stepName & ": " & message
    End If
End Sub
...
Sub ArchiveFile(sourcePath, destinationPath)
    ...
    If Err.Number <> 0 Then
        WScript.Echo "WARNING: Could not archive LX02 export: " & Err.Description
        Err.Clear
    End If
    On Error GoTo 0
End Sub
